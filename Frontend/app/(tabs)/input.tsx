...
export default function InputScreen() {
  ...
  const host = '10.158.161.107:3000'; // Updated to match your backend IP
  // ------------------------------------------------
  ...
  const updateStockPredictions = async (predictionResult: PredictionResult) => {
    try {
      console.log(
        'Updating stock predictions in database using bulk update...'
      );
  ...
  const updateStockPredictionsDirectly = async (
    ...
    try {
      ...
      Object.entries(predictionResult.predictions).forEach(
        ([productName, prediction]) => {
          // Find matching stock items
          const matchingStocks = stocksData.filter((stock) => {
            const stockProductName = stock.product_name?.toLowerCase();
      ...
      console.log(
        `Direct updates completed: ${successful} successful, ${failed} failed`
      );
  ...
  const predictDepletionDate = async (stockData: StockItem[]) => {
    ...
    try {
      ...
      if (result.prediction && result.prediction.predictions) {
        try {
          ...
          try {
            ...
            updateResult = await updateStockPredictionsDirectly(
              result.prediction,
              result.stocks
            );
          }
          ...
          Alert.alert(
            'Prediction Complete',
            `Depletion dates calculated for ${
              Object.keys(result.prediction.predictions).length
            } products.\n\nDatabase updated: ${
          ...
          Alert.alert(
            ...
            `Depletion dates calculated successfully!\n\nPredictions:\n${(
              Object.entries(result.prediction.predictions) as [
                string,
                ProductPrediction
              ][]
            )
              .map(
                ([product, pred]) =>
                  `• ${product}: ${pred.predicted_finish_date}`
              )
              .join(
                '\n'
      ...
      Alert.alert(
        'Prediction Error',
        `Could not calculate depletion dates: ${
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      if (response.data?.data && Array.isArray(response.data.data)) {
        const predictionMessages = response.data.data
          .map(
            (item: {
              product_name: any;
              predicted_finish_date: string | number | Date;
            }) =>
              `• ${item.product_name}: Predicted to finish on ${new Date(
  ...
  const handleSubmitText = async () => {
    ...
    setIsProcessingPrediction(true); // Start loading indicator

    try {
      // This single call now handles adding items AND getting predictions.
      const result = await stockService.addItemsToStock(manualText.trim());

      // This log will now finally appear!
      console.log('✅ Success from backend:', result);

      // Display the results to the user
      if (result?.data && Array.isArray(result.data)) {
        const predictionMessages = result.data
          .map(
            (item: {
              product_name: any;
              predicted_finish_date: string | number | Date;
        ...
        Alert.alert('Items Added & Predicted!', predictionMessages, [
          { text: 'OK' },
        ]);
      } else {
      ...
      setManualText(''); // Clear input field on success
    } catch (error) {
      ...
      setIsProcessingPrediction(false); // Stop loading indicator
    }
  };
        ...
        {userProfile && (
          <View style={styles.profileDataContainer}>
            ...
            <Text style={styles.profileDataText}>
              Region: {userProfile.region || 'Not set'}
            </Text>
            <Text style={styles.profileDataText}>
              Family: {userProfile.adult_male || 0} adult males,{' '}
              {userProfile.adult_female || 0} adult females,{' '}
            ...
            <View style={styles.familyField}>
              ...
              <TextInput
                ...
                onChangeText={(text) =>
                  setFamily((prev) => ({
                    ...prev,
                    adult_male: Number.parseInt(text) || 0,
                  }))
                }
            ...
            <View style={styles.familyField}>
              ...
              <TextInput
                ...
                onChangeText={(text) =>
                  setFamily((prev) => ({
                    ...prev,
                    adult_female: Number.parseInt(text) || 0,
                  }))
                }
                keyboardType="numeric"
                placeholderTextColor="#A0AEC0"
    ...
    <SafeAreaView style={styles.container}>
      <Animated.View entering={FadeInUp.delay(200)} style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Enter items manually or scan a receipt
        </Text>
        {userProfile && (
