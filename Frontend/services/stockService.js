...
class StockService {
  ...
  async preparePredictionPayload(additionalData = {}) {
    try {
      ...
      predictionData.stocks.forEach((stock, index) => {
        stockObject[`${stock.product_name.toLowerCase().replace(/\s+/g, "_")}`] = stock.quantity
      })
  ...
  async predictDepletion(additionalData = {}) {
    try {
      ...
      const response = await fetch("https://jannatul03-grocy-genie-grocy-genie-lstm.hf.space/stock/add", {
        method: "POST",
        headers: {
