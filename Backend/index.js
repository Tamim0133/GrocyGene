...
app.delete('/api/users/:id', async (req, res) => {
  const { error } = await supabase.from('user_stocks').select().eq('user_id', req.params.id);
  if (error) return res.status(500).json({ error: error.message });
  res.status(200).json({ message: 'User deleted successfully' });
});


// ---- Users Stocks Table (_D-One_)
app.get('/api/finish_date/:id', async (req, res) => {
  const { data, error } = await supabase.from('user_stocks').select('*').eq('user_id', req.params.id);
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
